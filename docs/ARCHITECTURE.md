...
- Supported provider types: `simulated` and `openai`
...
## UI provider selection and status flow (Phase 13B)
1. Workflow create/edit forms allow choosing `simulated` or `openai`; new workflows default to `simulated`.
2. The UI reads `GET /providers` to explain current provider availability without accessing secrets.
3. `simulated` reports `active` and is always the recommended public demo provider.
4. `openai` reports `disabled`, `missing_api_key`, or `enabled` based on backend configuration.
5. A workflow run still follows the same lifecycle and log contract regardless of the selected adapter.
6. Run details surface only safe provider metadata returned with successful output.
